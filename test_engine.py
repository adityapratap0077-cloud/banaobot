...
check("team se connect" in bodies(r)[0],
      "handoff follows the user's language (Hinglish msg -> Hinglish handoff)")
...
print("(j) per-message Hindi/English/Hinglish auto-detection")
send, store = make_client(10)
_, s, _ = send("i want to book a table for tonight")
check(s["lang"] == "en", "clear English sentence -> en")
_, s, _ = send("mujhe kal shaam ko table book karna hai")
check(s["lang"] == "hi", "Hinglish sentence -> hi (auto-switch, no selection)")
_, s, _ = send("please show me the full menu in english")
check(s["lang"] == "en", "back to English -> en again")
_, s, _ = send("मुझे मेन्यू दिखाओ")
check(s["lang"] == "hi", "Devanagari -> hi")
_, s, _ = send("ok")
check(s["lang"] == "hi", "neutral 'ok' keeps current language")
_, s, _ = send("cmd:menu")
check(s["lang"] == "hi", "button tap keeps current language")
r, s, _ = send("/english")
check(s["lang"] == "en" and s.get("lang_locked"), "/english locks language")
_, s, _ = send("mujhe paneer tikka chahiye")
check(s["lang"] == "en", "locked language survives Hinglish message")
r, s, _ = send("/hindi")
check(s["lang"] == "hi" and s.get("lang_locked"), "/hindi locks language")
check("भाषा" in bodies(r)[0] or "Hindi" in bodies(r)[0],
      "language confirmation shown in Hindi")
